...
function generateSvg(interactive=true){ const [w,h]=dims(state.format); let s=`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">${defsBlock()}`;
  ...
  if(currentTemplate==='country'){
    ...
    if(isMainPreset){
      ...
      s += renderLogo('mainLogo',interactive)+renderLogo('frp',interactive)+renderLogo('agency',interactive)+renderOptionalLogo('fipa',interactive)+renderOptionalLogo('prima',interactive)+renderText('titleLines',{fill:'#f3c739'},interactive);
      if(ov.show!==false){
